...
<html lang="ko">
	...
	<body>
		<div class="container">
			<div id="streams">
				${stream.map((s) => `<iframe src="${escapeQuote(s.player)}" name="${escapeQuote(isNaN(Number(s.id)) ? s.id : `#${s.id}`)}" frameborder="0" scrolling="no" allowfullscreen allow="autoplay; fullscreen; encrypted-media; local-network-access; loopback-network"></iframe>`).join('\n\t\t\t\t')}
			</div>
